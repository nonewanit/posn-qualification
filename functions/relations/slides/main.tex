...
% --- Document info ---
\title[ความสัมพันธ์และฟังก์ชัน]{ความสัมพันธ์และฟังก์ชัน}
...
\begin{frame}{ตัวอย่าง: การหาโดเมนและเรนจ์ (ต่อ)}
  \begin{solbox}[วิธีทำ (ต่อ)]
    ...
    $R_r = \{0, 1, 4\}$ \quad (ค่า $y$ ที่เกิดขึ้น โดยไม่นับซ้ำ)
  \end{solbox}
...
\begin{frame}{ตัวอย่าง: การหาค่าฟังก์ชัน}
  \begin{intuition}[สังเกต]
    ฟังก์ชันเหมือน \textbf{เครื่องผลิตน้ำผลไม้}: \newline
    ใส่ส้ม ($x$) เข้าไป $\rightarrow$ ได้น้ำส้ม ($f(x)$) ออกมา\newline 
    ใส่แอปเปิ้ล เข้าไป $\rightarrow$ ได้น้ำแอปเปิ้ล ออกมา
  \end{intuition}
...
\begin{frame}{ฟังก์ชันหนึ่งต่อหนึ่ง (One-to-One / Injective)}
  ...
  \begin{intuition}[สังเกต]
    ง่าย ๆ คือห้ามคู่ใครซ้ำกับคู่อื่น
  \end{intuition}
\end{frame}
...
\begin{frame}{ฟังก์ชันหนึ่งต่อหนึ่งและทั่วถึง (Bijective)}
  ...
  \begin{intuition}[สังเกต]
    ง่าย ๆ คือ ฝั่งโคโดเมนต้องมีคู่ทั้งหมด และคู่ใครคู่มันห้ามซ้ำ
  \end{intuition}
\end{frame}
