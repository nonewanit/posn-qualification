...
\title[รูปสองมิติ]{\textcolor{white}{รูปสองมิติ} \textcolor{boxBlue!60}{(2D Shapes)}}
\subtitle{รูปหลายเหลี่ยม • รูปนูน/เว้า • พื้นที่ • เส้นรอบรูป }
...
\begin{frame}{จำนวนเส้นทแยงมุม}
  ...
  \begin{intuition}[ทำไม?]
    จากแต่ละจุดยอดลากไปอีก n-3 จุด (ไม่ใช่จุดตัวเองและจุดข้าง ๆ) $\times n$ แล้วหาร 2 (แต่ละเส้นจะโดนนับซํ้าไปกลับ)
  \end{intuition}
\end{frame}
  ...
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{tikzpicture}[scale=1.0]
        ...
        \draw[boxPurple, thick, <->] (-0.2,0) -- (0.5,1.5) node[midway, left] {$a$};
      \end{tikzpicture}
...
\begin{frame}{สี่เหลี่ยมคางหมูและสี่เหลี่ยมรูปว่าว}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      \centering
      \begin{tikzpicture}[scale=1.0]
        \draw[thick, fill=boxGreen!8] (1.5,0) -- (4,1.2) -- (1.5,2.5) -- (0.2,1.2) -- cycle;
        \draw[boxBlue, dashed] (1.5,0) -- (1.5,2.5) node[midway, left] {$d_1$};
        \draw[boxOrange, dashed] (0.2,1.2) -- (4,1.2) node[midway, above] {$d_2$};
        % Right angle marker
